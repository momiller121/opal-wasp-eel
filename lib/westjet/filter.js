...
const filterDateBasedOnExcludes = function ( date, excludes ){
    ...
    for ( let i = 0; i < excludeList.length && included; ++i ){
        ...
        switch ( type ){
            case 'date' :
                const test = new Date( excludeList[ i ].replace( /-/g, '/' ) );
                if ( date.getTime() === test.getTime() ){
                    included = false;
                }
                break;
            case 'range' :
                const start = new Date( excludeList[ i ].split( 'to' )[ 0 ].trim().replace( /-/g, '/' ) );
                const end = new Date( excludeList[ i ].split( 'to' )[ 1 ].trim().replace( /-/g, '/' ) );
                if ( date.getTime() > start.getTime() && date.getTime() < end.getTime() ){
                    included = false;
                }
                break;
            case 'day of week' :
                const pos = config[ 'days-of-week' ].indexOf( excludeList[ i ] );
                if ( date.getDay() === pos ){
                    included = false;
                }
                break;
...
const filterPricingAPIResults = function ( results, start, end, min, max, excludes, sort ){
    ...
    for ( let i = 0; i < results.length; ++i ){
        ...
        const month = JSON.parse( results[ i ] );
        console.log( 'Processing month: ' + month[ 0 ].date );

        const days = Object.keys( month[ 0 ].day );
        console.log( '\tThere are ' + days.length + ' to process' );
        for ( let j = 0; j < days.length; ++j ){
            const day =  month[ 0 ].day[ days[ j ] ];

            if ( day.status === 'available' ){
                const cur =  month[ 0 ].date + '-' + day.day.padStart( 2, '0' );
    ...
    if ( sort ){
        filteredResults.sort( (a, b) => {
            ...
            // when the price is the same, sort by day:
            return a.price - b.price || a.day > b.day;
        });
    }
