...
server.route({
    method: 'GET',
    path:'/pricing/filtered/{ORIG}/{DEST}/{START}/{END}/{MIN}/{MAX}/{EXCLUSIONS?}',
    options: {
        description: 'example price service',
        notes: 'POC example exploring what might be required on a flaible price service endpoint',
        ...
        validate: {
            params: {
                ...
                START: Joi.string().required().description('Start date').default(dateDaysFromNow(15)),
                END: Joi.string().required().description('End date').default(dateDaysFromNow(30)),
            ...
            failAction: (request, h, error) => {
                ...
                console.error(error);
                const response = h.response({ foo:'bar', error });
